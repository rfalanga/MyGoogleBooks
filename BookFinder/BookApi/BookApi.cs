...
namespace BookApi
{
    public static class BookApi
    {
        ...
        public static async Task<Root?> GetBooksByTitle(string htmlEncodedTitle)
        {
            using HttpClient client = new HttpClient();
            try
            {
                string authorNamesEncoded = $"https://www.googleapis.com/books/v1/volumes?q=intitle:{htmlEncodedTitle}";

                HttpResponseMessage? responseMessage = await client.GetAsync(authorNamesEncoded);
